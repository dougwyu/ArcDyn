...
# [1] "sample"       "sp"           "PC"           "mapped_reads"
# sample:  sample information
# sp:  species code
# PC:  percent coverage of mapped reads. for mitogenomes, if the mapped reads cover >10% of the mitogenome, we consider it a true positive detection. for barcodes, we consider 50% PC (pct coverage) to be the threshold of detection. These values were derived from experiment with mock samples of known composition
# mapped_reads:  number of reads that mapped to a given barcode or mitogenome sequence that had mapping quality greater than a threshold value (I think i set this at MAPQ≥48 but check the paper)
```
...
env_data_mitogenome2 <- left_join(env_data_mitogenome2, species) %>% 
    dplyr::filter(PC >= 0.1) %>% # filters out false-positive detections
    count(mitogenome) %>% 
    arrange(desc(n))
