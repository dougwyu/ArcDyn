...
The code is a bit confusing because i have a choice of which dataset to use for the 'mitogenomes' dataset.  I always include the 406 barcodes dataset, and then i have a choice of including the 308mitogenomes or the 349MITOCOICYTB dataset.  (and to make it a bit more confusing, i can choose between the 20190202 run (minimap2 2.10) or the 20190204 run (minimap2 2.15) for the 349MITOCOICYTB dataset). For now, i am going with the 349MITOCOICYTB_20190204 dataset.  

To reduce code changes, after i load the 349MITOCOICYTB_20190204, i call it the 'mitogenomes' dataset, since the 349MITOCOICYTB_20190202/4 and the 308mitogenomes dataset are both mainly made up of mitogenomes.
...
Load saved idx_meta_genomecov_ABA2B2EFGH files, barcodes, mitogenomes, and MITOCOICYTB
```{r}
...
Make table of mitogenome names for 308 mitogenomes dataset.
...
Make table of MITOCOICYTB names for 349 MITOCOICYTB dataset
```{r}
# idx_meta_genomecov_ABA2B2EFGH_mitogenomes <- idx_meta_genomecov_ABA2B2EFGH
#  
# rm(idx_meta_genomecov_A2B2, idx_meta_genomecov_AB, idx_meta_genomecov_EF, idx_meta_genomecov_GH, idx_meta_genomecov_ABA2B2EFGH) 
...
COI_CytB_ACX5953_Diptera_Chironomidae_Diamesa_arctica_COIsameAsACW5472 has two BOLD numbers:  ACW5472 and ACX5953.  ACW5472 is used in the Barcodes database and ACX5953 is the BOLD number in the mitogenomes database

to allow full merging of the two datasets, i need to replace ACX5953 with ACW5472 in the mitotaxonomies dataset
...
# env_data_MITOCOICYTB dataset #  will save as env_data_mitogenome
```{r}
...
Load saved mocks_idx_meta_genomecov files.  The minimap2_349MITOCOICYTB_20190204 dataset is the one that we now want for 
```{r}
barcodesoutputidxstatstabulatefolder <- "../../PlatesABA2B2EFGH/minimap2_406barcodes_20190116" 
# mitogenomesoutputidxstatstabulatefolder <- "../../PlatesABA2B2EFGH/minimap2_308mitogenomes_20190115"
MITOCOICYTBoutputidxstatstabulatefolder <- "../../PlatesABA2B2EFGH/minimap2_349MITOCOICYTB_20190204"
...
Save datasets into a single RData file, checking that the filename doesn't already exist
```{r}
# setwd("~/Dropbox/Working_docs/Roslin_Greenland/2017/bulk_samples/ArcDyn_scripts/7_ecological_analysis/Rdata")
# getwd()
mainDir = "~/Dropbox/Working_docs/Roslin_Greenland/2017/bulk_samples/ArcDyn_scripts/7_ecological_analysis/Rdata"
filename = "input_data_step5_20190204.RData"
...
Loading input_data_step5_20190120.RData, which is correctly formatted for downstream statistical analysis. I used these datafiles as my template examples for the above code. For viewing convenience, I then put these files into an Excel spreadsheet (saved to the manuscript_methods/ folder)
