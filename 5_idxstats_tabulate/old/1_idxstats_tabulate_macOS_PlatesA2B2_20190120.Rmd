...
samtoolsfilter <- "F2308_f0x2" # f0x2 is PROPER PAIR
samtoolsqual <- "q48"
...
samplemetadatafolder <- "~/Dropbox/Working_docs/Roslin_Greenland/2017/bulk_samples/ArcDyn_scripts/6_referenceseqs_metadata/PlatesAB_metadata"  # this is the enclosing folder around the sample metadata spreadsheet
...
idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count() %>% arrange(desc(n)); View(idx_mitogenome_table) # mitogenomes: all columns should be 311 (308 + 3);  barcodes:  all cols should be 409 (406 + 3)
...
# rm(species)
# 53181 rows / 311 (mitogenomes + COIspikes) == 171 samples
# 69939 rows / 409 (barcodes + COIspikes) == 171
```
...
The Earlham Institute fastq filenames for PlatesA2B2 are different from the filenames used in the first RUN (PlatesAB). In PlatesA2B2, they use ArcDyn's sample names:  PlatesAB, no longer using EI names, e.g. IPO3916, IP3917.  So i have to do some formatting of the samplemetadata to produce sample names in the format:  PlateA_A1, ...
```{r 3. left_join sample metadata to idx table}

samplemetadata <- samplemetadata %>% rename(EI_sample="sample") # change column name "sample" to "EI_sample."  This column holds the sample name in the format IPO3916_A1.
...
# [16] "EI_Plate_name"              "EI_sample"                  "TGAC_barcode"              
...
loadFile2 <- function(x) {
    ...
    # originally, i hand-set col_types, but very large numbers are returned in sci notation by genomecov 
    # and this requires col_double(). So now I let R parse column types. it is able to read sci notation.
    # If i really wanted to set by hand, i should use col_double(), like it is written here.
    # col_types = cols(
    #     mitogenome = col_character(),
...
# read in filename, extract and sub in the first bit of the name, remove any filepaths (using basename) and store in df$sample.  example filename:  Sample_PRO1322_PlateA_A1_F2308_f0x2_q1_sorted_genomecov_d.txt.gz
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesA2B2_20190115_308mitogenomes.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_PlatesA2B2_20190115_308mitogenomes.RDS")

# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesA2B2_20190116_406barcodes.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_PlatesA2B2_20190116_406barcodes.RDS")
...
# [16] "ArcDyn_Plate_name"          "EI_Plate_name"              "EI_sample"                 
# [19] "TGAC_barcode"               "Concentration_ng_per_ul"    "Sample_volume_ul"          
...
COI_spike_reads %>% distinct(Full_name_of_the_sample) %>% count()  # number of distinct values of Full_name_of_the_sample: should be 171
COI_spike_reads %>% distinct(Full_name_of_the_sample) %>% arrange(Full_name_of_the_sample) # number of distinct values of Full_name_of_the_sample: should be 171
...
# [17] "EI_Plate_name"                               "EI_sample"                                  
# [19] "TGAC_barcode"                                "Concentration_ng_per_ul"                    
...
# filter out samples with COI_spike_sum < 200000 (summed over all 308 mitogenomes or 406 barcodes in the sample).  The largest value of elaterid_mordellid_sum is 16,727,485,975, so this keeps the COI_corr from getting too large (and thus causing the COI_corr reads from getting too small)

sample_names <- sum_idx_meta_genomecov %>% dplyr::filter(COI_spike_sum < 200000) %>% dplyr::select(Full_name_of_the_sample)
...
idx_meta_genomecov <- idx_meta_genomecov %>% filter(!Full_name_of_the_sample %in% samples_to_remove) # 49588 obs
...
# should be 2, and it's 1.85, sd = 0.28
...
# should be 4, and its 3.44, sd = 0.72
...
# should be 2, and it's 1.84, sd = 0.14
...
```{r}
# calculate COI_correction factor:  elaterid_mordellid_bombyx_sum/min(elaterid_mordellid_bombyx_sum)
...
```{r create variable from Date and Trap(ABC)}
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesA2B2_20190115_308mitogenomes.txt") # so if i need to read in a file during debuggging, here it is
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesA2B2_20190116_406barcodes.txt") # so if i need to read in a file during debuggging, here it is

# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesA2B2_20190115.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesA2B2_20190116_406barcodes.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
#
```
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesA2B2_20190115_308mitogenomes.txt")
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesA2B2_20190116_406barcodes.txt")
...
# uncomment to save table
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_pctcovmin_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_pctcovmin_wide_PlatesA2B2_20190115_308mitogenomes.txt")
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_pctcovmin_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_pctcovmin_wide_PlatesA2B2_20190116_406barcodes.txt")
```
